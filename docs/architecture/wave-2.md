...
- an English read-only personal-inventory NUI opened with F2;
...
`cnr_items` owns definition state. Codes, labels, descriptions, weights, stack limits, flags, handlers, and metadata schema versions are server data and are never accepted from a client.
...
The client cannot submit account, session, character, definition, item instance, metadata, weight, capacity, target slot, inventory version, distance, or result state. The first UI exposes snapshots only; the transfer contract is reserved for later server-presented secondary inventories.
