...
`20260716001300`. Apply `20260716001400_dynamic_atms_cash_operations.sql` and set the minimum schema
to `20260716001400`. Start `cnr_banking` after `cnr_permissions` and `cnr_characters`, and before
`cnr_ui`. Starter cash, checking amounts, and maximum transfer/ATM amounts are migration-backed server settings. The Tablet
...
The ATM migration seeds one development terminal at Legion Square Parking. Active terminals are
otherwise managed without a restart by an in-game account holding `banking.atms.manage`:
...
The server derives the administrator, position, heading, radius, creator, and status. The terminal NUI
opens only after the server validates an active FULL session, spawned character, active ATM, and
server-observed proximity. Deposit and withdrawal intents contain only ATM UUID, direction, positive
minor-unit amount, request ID, operation UUID, and contract version.
...
```cfg
set cnr_schema_minimum "20260716001400"
...
set cnr_banking_atm_radius "2.5"
set cnr_banking_atm_admin_radius "8.0"
...
At the seeded parking ATM, enter the marker and press the context control (`E`). Confirm the English
ATM NUI shows Cash Wallet and Personal Checking balances. Deposit part of the wallet, withdraw part of
checking, and verify each confirmed action updates both balances without closing the terminal. Retry
the same operation after interrupting its response and confirm it resolves once. Walk out of range and
confirm a new mutation fails without moving money. An owner or administrator can create another ATM at
their current position with `/cnr_atm_create Downtown ATM`, inspect it with `/cnr_atm_list`, and
deactivate the nearest terminal with `/cnr_atm_remove`.
