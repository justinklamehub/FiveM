...
After a controlled spawn, press F2 or run `cnr_inventory_open` in the client F8 console. The English personal inventory must show exactly two Water Bottles, two Sandwiches, and the existing State Identification Card on first provisioning. Reopening or reconnecting must not duplicate them. At the configured parking locker, press F3 or run `cnr_storage_open`. The server must open the two-panel `Inventory & Locker` workspace only within the configured radius; cross-panel drops move complete stacks into a server-selected slot and persist after reconnect.
